function Test-Menu()
{
    ...
    if([Linq.Enumerable]:: SequenceEqual($x, $SIG))
    {
        ...
        {
            [byte[]] $FAT = [byte[]]::new($NumFATSects*$Sectorsize)
            $m=0
            for ($i=0;$i -lt 109;$i++)
            {
                $DiFATEntry = [System.BitConverter]::ToUInt32($b,76+4*$i)
                $FATStart = ($DiFATEntry + 1 ) * $sectorsize
                [byte[]]$arr = $b[$FATstart..($FATStart+$sectorsize-1)]
                $arr.CopyTo($FAT,$m)
                $m+=$Sectorsize
            }
            $DiFATStart = ($DiFATSect1+1)* $Sectorsize
            Write-Debug "DiFATStart: $DiFATStart"
            $n=0
...
Test-Menu "C:\TEMP\CS0024669\test10.XLS" -Debug 
